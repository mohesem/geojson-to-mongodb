...
mongoose.connection.on("connected", () => {
  ...
  fs.createReadStream(`./layers/2.geojson`).pipe(
    // /* COUNTRIES */
    // geojsonStream.parse((country, index) => {
    //   GeoModelBbox.findOne({$and: [
    //             { name0: country.properties.NAME_0 },
    //             { name1: country.properties.NAME_1 }
    //           ]}, (err, doc) => {
    //     if (err) return err;
    //     if (doc) {
    ...
    /* LEVEL_2 */
    geojsonStream.parse((level_2, index) => {
      GeoModelBbox.findOne(
        {
          $and: [
            { name0: level_2.properties.NAME_0 },
            { name1: level_2.properties.NAME_1 },
            { name2: level_2.properties.NAME_2 }
          ]
        },
        (err, doc) => {
          if (err) return err;
          ...
          if (!doc) {
            const newCoord = turf.cleanCoords(level_2).geometry.coordinates;
            const multiPoly = turf.multiPolygon(newCoord, {
              name: level_2.properties.NAME_2
            });
            const bbox = turf.bbox(multiPoly);
            const bboxPolygon = turf.bboxPolygon(bbox);
            const newGeoModelBbox = new GeoModelBbox({
              name0: level_2.properties.NAME_0,
              name1: level_2.properties.NAME_1,
              name1: level_2.properties.NAME_2,
              geo: bboxPolygon.geometry
            });
            ...
            newGeoModelBbox.save(err => {
              if (err) console.log(err);
              console.log(
                `${level_2.properties.NAME_2} in ${level_2.properties.NAME_1} in ${level_2.properties.NAME_0} bbox saved`
              );
            });
